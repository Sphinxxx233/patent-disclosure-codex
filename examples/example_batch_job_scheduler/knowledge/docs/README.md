...
**Step 2 扫描约定**（见 `prompts/project_scan.md`）：Codex **须**先将下表 `.docx`/`.pptx` **转为 `.md` 再读取**，不可只扫 `architecture.md` 而忽略 Office；**勿**对 `sample_assets/*.png` 单独做识图（与内嵌图重复，以转换后的 Markdown 为准）。
