...
与 `merger.md` 相同：先 **读取** **`prompts/iteration_context.md`**，再**读取** 当前定稿与纠正意图；纠正结果须**另存为** **`{案件名}_{YYYYMMDDHHmmss}.md`** 及同名 `.docx`（§7.3 第 5 点），**禁止**默认覆盖旧稿。**禁止**在迭代意图下无合并/纠正落盘却回到 Step 3–4 全文专利点分析（除非用户明确要求重做专利点）。

## 何时启用

由 Codex 根据用户**意图**判断：用户针对**已有交底书**指出**错误、与事实或参数不符、表述问题、保护点调整**等（例如「这里不对」「和 3.5 不一致」「保护点应强调 XXX」）时，**应**执行本流程。**不要求**用户说出「迭代」等固定词；也**不必**先询问是否进入迭代模式。
